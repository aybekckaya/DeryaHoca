...
-(NSDictionary *) objectTypes
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (strcmp(rawPropertyType, @encode(float)) == 0) {
            //it's a float
            [dctMute setObject:@"float" forKey:propertyName];
            
        } else if (strcmp(rawPropertyType, @encode(int)) == 0) {
            //it's an int
              [dctMute setObject:@"integer" forKey:propertyName];
            
        }else if(strcmp(rawPropertyType, @encode(long)) == 0)
        {
            // long
               [dctMute setObject:@"long" forKey:propertyName];
        }
        ...
        if ([typeAttribute hasPrefix:@"T@"]) {
            ...
            if (typeClass != nil) {
                ...
                [dctMute setObject:@"NSString" forKey:propertyName];
...
-(void)setDictionary:(NSDictionary*)dictionary
{
        ...
        if (![obj isMemberOfClass:[NSNull class]] && ![key isEqualToString:@"id"]) {
            @try {
                ...
                if (property) {
                   ...
                   // //NSLog(@"property Type : %@" , properyType);
    ...
    ////NSLog(@"des : %@" , [self debugDescription]);
