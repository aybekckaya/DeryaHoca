...
-(id)Query:(NSString *)query
{
    ...
    NSRange RngShow = [TFStrings SearchString:@"SHOW" InString:query options:NSCaseInsensitiveSearch];
    
     NSRange RngDescribe = [TFStrings SearchString:@"DESCRIBE" InString:query options:NSCaseInsensitiveSearch];
    
    // TFStrings
    
    if(Rng.length>0 || RngShow.length > 0 || RngDescribe.length > 0)
    {
        // Select Query
        [database open];
...
-(NSArray *)allColumnNames:(NSString *)tableName
{
    ...
    while ([columnNamesSet next]) {
        ...
        NSString *strColumnName = [columnNamesSet stringForColumn:@"name"];
        [arrColumnNames addObject:strColumnName];
        
    }
    
    return [[NSArray alloc]initWithArray:arrColumnNames];
}
